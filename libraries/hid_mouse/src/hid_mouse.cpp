...
static const PROGMEM char usbHidReportDescriptor[52] = { /* USB report descriptor, size must match usbconfig.h */
...
void HidMouse::begin(bool cal_osc) {
...
void HidMouse::calibrateOSC(void) {
//	cli();
//	calibrateOscillator();
//	sei();
};

void HidMouse::loop(void) {
...
//void HidMouse::OnMsg(void (*onMsgCallback)(TFMidiMessage)) {
//	_onMsgCallback = onMsgCallback;
//};

/*
void HidMouse::refresh() {
	wdt_reset();
	processMessage();
	usbPoll();	
};

void HidMouse::read(uchar *data, uchar len) {
	cli();
	buffPush(data);
	if (len > 4) {
		buffPush(data+4);
...
//void HidMouse::write(TFMidiMessage msg) {
//	byte buffer[4];
//	buffer[0] = msg.type >> 4;
...
//};
//
//void HidMouse::write(byte *buffer, byte size) {
